...
class QueueService {
  ...
  #init = () => {
    ...
    if (!queueName || !redisConfig) throw new Error(`Missing queue configuration`);

    this.#queue = new Queue(queueName, { connection: redisConfig });
    this.#scheduler = new QueueScheduler(queueName, { connection: redisConfig });
  };

  publishMessage = async (payload, delay = 0, options = {}) => {
    return this.#queue.add("job", payload, { delay, removeOnComplete: true, ...options });
  };

  publishToBullInBatches = async (formattedDataArr, delay = 15000, batchSize = 20) => {
    ...
    for (const formattedData of formattedDataArr) {
      ...
      if (existingBatch) {
        ...
        await this.#queue.add("batch-job", [formattedData], { removeOnComplete: true, delay });
      }
    }
  };
...
module.exports = { QueueService, serviceTypes };