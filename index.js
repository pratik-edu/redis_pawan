 ...
 */
function pawan(host, port, serviceName, password, options) {
  ...
  const setGlobalKey = async (key, data, expiry) => {
    const compressedData = compress(data);
    const setSucc = await setAsyncGlobal(key, compressedData, "EX", expiry || EXPIRY);
    return setSucc;
  };
  ...
  const generateKey = (parseKey = defaultKey) => {
    return (req, res, next) => {
      req.__cache_key = typeof parseKey === 'function' ? parseKey(req) : parseKey;
      next();
    }
  }
   ...
   */
  const cacheFn = (fn, keyGenerator, ttl = EXPIRY, { redisKeyPrefix = 'cacheFn:', isGlobal = false } = {}) => {
    const getKeyFn = isGlobal ? getGlobalKey : getKey; 
    const setKeyFn = isGlobal ? setGlobalKey : setKey; 
    if (!(fn && keyGenerator && ttl)) throw new Error(`need all 3 arguments fn, keyGenerator and ttl.`)
    return async (...args) => {
      ...
      const redisData = stringifiedRedisData ? JSON.parse(stringifiedRedisData) : null;
      if (redisData) return redisData;
      else {
        const data = await fn(...args);
        if (key)
          await setKeyFn(key, JSON.stringify(data), ttl);
        return data;
      }
    };
  };
